...
from amm_tool_extractor.extractor.html_parser          import TaskData, ToolEntry, ConsumableEntry, ExpendableEntry
from amm_tool_extractor.extractor.crawler              import NOT_FOUND_MARKER
from amm_tool_extractor.extractor.stock_checker        import StockChecker, StockResult, _parse_part_numbers
from amm_tool_extractor.extractor.consumable_list_parser import ConsumableListParser
...
def _write_support_equipment(wb, all_tasks, stock_checker, tool_list):
        ...
        for tool in task.tools:
            ...
            if rid not in dedup:
                dedup[rid] = {"description": tool.description,
                              ...
                              "effectivity": tool.effectivity,  # NEW FIELD
    ...
    for ref_id in sorted(dedup.keys(), key=sort_key):
        ...
        # Extract effectivity from tool list OR from parsed data
        if tl:
            effectivity_list = [pn.get("effectivity", "") for pn in tl.part_numbers]
        else:
            effectivity_list = entry.get("effectivity", [])

        # Build effectivity display text
        effectivity_text = "\n".join(eff for eff in effectivity_list if eff) if effectivity_list else "—"
            ...
            if result.found:
                for i, rec in enumerate(result.records):
                    ...
                    values = [
                        ...
                        effectivity_text if i == 0 else "",  # NEW COLUMN
                        titles if i == 0 else "",
                        count if i == 0 else "",
                ...
                values = [ref_id, description, pn_display, effectivity_text,  # NEW COLUMN
                          titles, count, "NOT FOUND", "—", "—", "—", 0]
                for col, val in enumerate(values, 1):
                    cell = ws.cell(row=row_idx, column=col, value=val)
                    _dat(cell, bg=STOCK_NO_BG, bold=(col == 1))
                    if col == 7:  # Adjusted column index for "NOT FOUND"
                        cell.font = Font(name="Arial", size=9, bold=True, color=NOT_FOUND_FG)
                row_idx += 1
        else:
            bg = ALT_BG if row_idx % 2 == 0 else None
            values = [ref_id, description, pn_text, effectivity_text,  # NEW COLUMN
                      titles, count]
            for col, val in enumerate(values, 1):
                _dat(ws.cell(row=row_idx, column=col, value=val),
                     bg=bg, bold=(col == 1))
    ...
    if has_stock:
        _widths(ws, {"A": 14, "B": 40, "C": 42, "D": 20,  # D is new A/P Effectivity
                     "E": 40, "F": 10, "G": 20, "H": 10, "I": 10, "J": 14, "K": 8})
    else:
        _widths(ws, {"A": 14, "B": 40, "C": 42, "D": 20,  # D is new A/P Effectivity
                     "E": 40, "F": 10})
