...
class ToolEntry:
    ...
    source_dmc: str
    source_title: str
    effectivity: List[str] = field(default_factory=list)  # NEW FIELD
    ...
    if supp_heading:
        ...
        if supp_table:
            for row in supp_table.find_all("tr"):
                ...
                # Extract part numbers AND effectivity
                part_numbers = []
                effectivity_list = []
                for d in desc_cell.find_all("div", class_="line"):
                    line_text = d.get_text(strip=True)
                    if line_text:
                        part_numbers.append(line_text)
                        # Extract A/P Effectivity from the line
                        eff_match = re.search(r'A/P Effectivity:\s*([^)]+)', line_text)
                        if eff_match:
                            effectivity_list.append(eff_match.group(1).strip())
                        else:
                            effectivity_list.append("")  # No effectivity for this part number
                ...
                tools.append(ToolEntry(
                    ...
                    source_title=title,
                    effectivity=effectivity_list,  # NEW FIELD
                ))
